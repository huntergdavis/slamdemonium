...
Bodies added after boot go through `createSurfacedBodies(world, registry)` in `surfacedBodies.ts`, which registers the authored surface in the same call that creates the body; pass the same `createSurfaceRegistry()` instance to `track.createSurfaceResolver(trackBodies, registry)` so the resolver sees them. `loopDeLoop.ts` authors the loop-de-loop as data (a helix of pitched slabs; a planar loop lands on its own entry) through the same facade, with clearance and entry-speed integration tests. `ramps.ts` authors ramps as data and installs them through the facade at boot (collider and mesh from one descriptor; the clearance and jump integration tests guard the example routes and the grip). `bodyPool.ts` reserves 192 breakable bodies and 768 debris bodies at boot (`POOL_BUDGET`, with its arithmetic); the current authored records promote only the nearby subset, and no body is created or destroyed mid-session.
