...
import { PARAM_BY_KEY } from '../tuning/schema';

export interface TireWheelReading {
  alpha: number; // radians, matching vehicle telemetry
  gripUsage: number;
  Fz: number; // positive contact load; zero while airborne
}
export interface TirePlotTelemetry {
  wheels: readonly TireWheelReading[];
} // FL, FR, RL, RR
...
export function tireCurve(
  ...
  return x <= 1
    ? 2 * x - x * x
    : slideRatio + (1 - slideRatio) * Math.exp(-falloff * (x - 1));
}

/** Reuses out; unsupported/airborne wheel readings do not fabricate zero-usage dots. */
export function readOperatingPoints(
  telemetry: TirePlotTelemetry | undefined,
  out: OperatingPoints,
): void {
  let frontCount = 0;
  let rearCount = 0;
  ...
  for (let index = 0; index < 4; index++) {
    ...
    if (
      !wheel ||
      !Number.isFinite(wheel.Fz) ||
      wheel.Fz <= 0 ||
      !Number.isFinite(wheel.alpha) ||
      !Number.isFinite(wheel.gripUsage) ||
      wheel.gripUsage < 0
    )
      continue;
    const point = index < 2 ? out.front : out.rear;
    point.slipDegrees += (Math.abs(wheel.alpha) * 180) / Math.PI;
    point.usage += wheel.gripUsage;
    if (index < 2) frontCount++;
    else rearCount++;
  }
  out.front.visible = frontCount > 0;
  out.rear.visible = rearCount > 0;
  if (frontCount) {
    out.front.slipDegrees /= frontCount;
    out.front.usage /= frontCount;
  }
  if (rearCount) {
    out.rear.slipDegrees /= rearCount;
    out.rear.usage /= rearCount;
  }
}
...
export class TireCurvePlot {
  ...
  constructor(
    ...
    private readonly readTelemetry: () => TirePlotTelemetry | undefined = () =>
      undefined,
  ) {
    ...
    this.canvas.setAttribute(
      'aria-label',
      'Tire response: slip angle in degrees versus normalized grip usage',
    );
    this.canvas.setAttribute('role', 'img');
    this.canvas.textContent =
      'Front solid and rear dashed tire curves; peak grip is 1.0.';
    ...
    this.colors = {
      background: token('--sl-panel'),
      grid: token('--sl-border'),
      text: token('--sl-muted'),
      front: token('--sl-accent'),
      rear: token('--sl-drift'),
      peak: token('--sl-warning'),
    };
    this.resize(this.width);
    this.unsubscribe = store.onChange((change) => {
      if (PARAM_BY_KEY[change.key].group === 'Tires') this.draw();
    });
  ...
  update(nowMs: number, visible = true): boolean {
    ...
    this.caption.nodeValue =
      this.points.front.visible || this.points.rear.visible
  ...
  private describe(point: OperatingPoint): string {
    return point.visible
      ? `${point.slipDegrees.toFixed(1)}° / ${point.usage.toFixed(2)}`
      : 'AIR';
  }
  ...
  private draw(): void {
    ...
    for (let axle = 0; axle < 2; axle++) {
      ...
      for (let index = 0; index <= 180; index++) {
        ...
        const y =
          bottom - (tireCurve(slip, peak, slide, falloff) / 1.5) * plotHeight;
      ...
      if (point.visible) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, axle === 0 ? 5 : 3, 0, Math.PI * 2);
        ctx.fill();
