...
export class OptionsPanel {
  ...
  constructor(private readonly options: OptionsPanelOptions) {
    ...
    this.opener.style.cssText = 'position:fixed;top:var(--sl-safe);right:var(--sl-safe);z-index:19';
    this.opener.setAttribute('aria-controls', 'sl-options');
    this.opener.setAttribute('aria-expanded', 'false');
    this.opener.addEventListener('click', () => { this.setOpen(!this.open); });
    ...
    const heading = node(doc, 'div', 'sl-toolbar');
    const title = node(doc, 'h2', 'sl-options__title', 'Options');
    ...
    this.preset.addEventListener('change', () => {
      this.run(() => {
        ...
        if (value.startsWith('builtin:')) this.session.applyBuiltin(value.slice(8) as BuiltinPresetName);
        else if (value.startsWith('user:')) this.session.applyUser(value.slice(5));
      });
    });
    const save = node(doc, 'button', 'sl-button', 'Save as…');
    save.type = 'button';
    save.addEventListener('click', () => {
      const name = doc.defaultView?.prompt('Name this tuning preset', 'My tune');
      if (name !== null && name !== undefined) this.run(() => { this.session.saveAs(name); });
    });
    this.edited = node(doc, 'span', 'sl-badge', 'Edited');
    this.edited.dataset.state = 'edited';
    presets.append(this.preset, save, this.edited);
    ...
    search.addEventListener('input', () => { this.groups.search(search.value); });
    header.append(comparison, node(doc, 'p', 'sl-caption', 'O opens Options. Tab swaps A/B while driving; inside Options, Tab moves between controls.'), pauseLabel, search);
    const body = node(doc, 'div', 'sl-options__body');
    this.element.append(header, body);
    this.groups = new OptionsGroups(body, this.session, drivingSurface, options.readTelemetry);
    ...
    file.addEventListener('change', () => {
      ...
      if (!selected) return;
      void selected.text().then((json) => { this.run(() => { this.session.importJSON(json); }); })
        .catch(() => { this.notice = 'Could not read this file.'; this.sync(); });
      file.value = '';
    });
    const actions: readonly [string, () => void][] = [
      ['Export', () => { this.download(); }],
      ['Import', () => { file.click(); }],
      ['Share link', () => { void this.share(); }],
      ['Reset everything', () => { this.session.resetEverything(); }],
    ];
    ...
    this.unsubscribe = this.session.onUpdate((key) => { this.groups.sync(key); this.sync(); });
    this.sync();
  }
  ...
  update(nowMs: number): void {
    this.groups.plot.update(nowMs, this.open && this.groups.tires.open && !this.groups.tires.hidden);
  }
  ...
  private sync(): void {
    ...
    for (const name of session.persistence.listPresets()) {
      ...
      if (Array.from(this.preset.options).some((option) => option.value === value)) continue;
    ...
    if (Array.from(this.preset.options).some((option) => option.value === value)) this.preset.value = value;
    else {
      let imported = this.preset.querySelector<HTMLOptionElement>('[data-imported]');
      if (!imported) { imported = node(this.preset.ownerDocument, 'option'); imported.dataset.imported = 'true'; imported.append(this.preset.ownerDocument.createTextNode('')); this.preset.append(imported); }
      imported.value = 'current';
      if (imported.firstChild) imported.firstChild.nodeValue = session.presetName;
    ...
    const labels = { idle: 'Autosave ready', saving: 'Saving…', saved: 'Saved locally', unavailable: 'Local storage unavailable · export to keep this tune', error: 'Save failed · export to keep this tune' };
  ...
  private run(action: () => void): void {
    this.notice = '';
    try { action(); } catch (error) { this.notice = error instanceof Error ? error.message : 'Unable to apply this action.'; }
    this.sync();
  }

  private download(): void {
    const url = URL.createObjectURL(new Blob([this.session.exportJSON()], { type: 'application/json' }));
