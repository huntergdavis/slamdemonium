...
export interface ScriptInput extends GameInput {
...
export function parseScriptSpawn(value: unknown): ScriptSpawn {
  ...
  const q = Object.freeze({
    x: finite(rotation.x, 'rotation.x'),
    y: finite(rotation.y, 'rotation.y'),
    z: finite(rotation.z, 'rotation.z'),
    w: finite(rotation.w, 'rotation.w'),
  });
  if (Math.abs(Math.hypot(q.x, q.y, q.z, q.w) - 1) > 1e-5)
    throw new RangeError('Spawn rotation must be a unit quaternion.');
...
export function parseScriptInput(value: unknown): Readonly<ScriptInput> {
  ...
  if (typeof input.handbrake !== 'boolean' || typeof input.boost !== 'boolean')
    throw new TypeError('handbrake and boost must be booleans.');
  if (input.source !== 'keyboard' && input.source !== 'gamepad')
    throw new TypeError('Input source must be keyboard or gamepad.');
...
export function parseScriptHeader(value: unknown): ScriptHeader {
  ...
  if (raw.version !== 1)
    throw new RangeError('Unsupported input script version; expected 1.');
  if (typeof raw.name !== 'string' || !raw.name.trim())
    throw new TypeError('Script name must not be empty.');
  ...
  for (const key of Object.keys(values))
    if (!isParamKey(key)) throw new RangeError('Unknown tuning key: ' + key);
  const tuning = {} as ParamSet;
  for (const definition of PARAM_DEFS) {
    if (!Object.hasOwn(values, definition.key))
      throw new TypeError(
        'Full tuning header required; missing ' + definition.key,
      );
    const value = finite(values[definition.key], definition.key);
    if (normalizeValue(definition.key, value) !== value)
      throw new RangeError('Invalid replay tuning value: ' + definition.key);
  ...
  return Object.freeze({
    ...
    name: raw.name,
    seed,
    spawn: parseScriptSpawn(raw.spawn),
...
export function parseInputScript(source: unknown): InputScript {
  ...
  if (!Array.isArray(raw.frames) || raw.frames.length === 0)
    throw new TypeError('Script must include input at step 0.');
  ...
  for (const entry of raw.frames) {
    ...
    if (
      ...
      throw new RangeError(
        'Frames must start at zero, increase strictly, and precede durationSteps.',
      );
