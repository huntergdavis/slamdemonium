...
Five continuous voices (procedural engine, three tyre profiles, boost sustain) reserve five of a hard fifteen-voice total. Impacts and boost attacks share ten explicit slots. Excess transients are dropped and counted. Howler's pool size is an inactive-object recycling setting, not the voice limit.

The engine is a procedural four-cylinder voice in an AudioWorklet (`engineProcessor.ts`, loaded by `EngineSynth` from the activated backend, so it sits behind the same activation gate and requests nothing before interaction). Its RPM is synthetic from speed and throttle, scaled by the slow-motion rate; the `engineCharacter` Audio tuning (0..1, default 1) blends an even four-cylinder (idle 900, about 7100 at the note ceiling) into a slower, bassier muscle voice with uneven firing (idle 650, about 4400); the vehicle has no gearbox or measured RPM. No engine sample ships, so there is no loop seam. The worklet output feeds Howler's master gain, so master mute, sfxVolume and the pause fade apply to it; `process` allocates nothing. A browser without AudioWorklet leaves the engine silent and the game usable. Grounded contact speed, lateral slip, load, locked wheels and bounded wheelspin feedback drive the tyre layers, with 40 ms attack / 180 ms release. A handbrake button alone cannot produce airborne screech. Wheel material is canonical `wheel.surfaceId`, never raw `wheel.hit.surfaceId` or an X/Z geometry guess. Unresolved/contact-only wheel surfaces stay silent, never default to asphalt. Existing sample families use distinct runtime pitch profiles; they are a first sound pass, not physically measured material recordings.
