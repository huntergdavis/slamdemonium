...
- [ ] WP19 | developer 2 + designer | **First-class controller support (CTO, high priority).** Today the pad drives and opens the menu but cannot reach most of the game: pad has 6 bindings (throttle, brake, steer, handbrake, boost, respawn, pause) against 13 keyboard actions. Missing from pad entirely: HUD mode, telemetry record, slow motion, A/B swap, gizmos, camera preset, latency probe, and **Options sliders cannot be adjusted at all without a mouse or keyboard**. Scope: (a) every action reachable from the pad; (b) Options page fully navigable AND adjustable by pad, including sliders and the search/preset controls; (c) visible, unambiguous focus everywhere (designer owns); (d) on-screen prompts show pad glyphs when a pad is active and key names otherwise; (e) haptics/rumble on impacts, kerbs and wheelspin; (f) graceful hot-plug connect/disconnect mid-session. | dev2/wp19-controller
- [ ] F0 | unassigned | **Shared surface definitions (research R7 finding, do WITH F1).** Ray hits carry `surfaceId` but tyre grip uses one global `surfaceGrip`, so grip, audio and FX can each tell a different story about what the tyres are touching. Resolve the id through ONE surface definition in content data, preserving the global multiplier, before any grass/gravel exists.
- [ ] F5 | unassigned | **Crash + retry loop (research R7: the list's biggest omission).** Design 1 promises a crash-happy racer and there is currently nothing to hit. One short smash route, light breakables, bounded debris, impact feel, scoring, instant retry. Contact impulse is nullable on our engine, so severity must be explicitly estimated. Playtest before traffic or deformation.
- [ ] F6 | unassigned | **Run loop + pose ghosts (research R7).** Start, countdown, checkpoint validity, finish, personal best, retry. Ghosts are cheap but NOT free: replay owns the player's input, so use a non-colliding pose ghost (~49 KiB/min) rather than simulating a second car. Version track, car, tuning and rules identity before trusting saved bests.
...
- [ ] F4 | unassigned | **Larger maps + generator (CTO). HIGHEST ARCHITECTURAL RISK per R7: do LAST.** Needs versioned `TrackDefinition` + one compiler for render/collision/lap, quaternion shape descriptors and scoped body removal in the physics adapter, and lap timing that does not assume a circle. Editor is a mode sharing the same compiler; do NOT fork a second world builder. Generate a track from a 2-D drawing for fast test maps, reproduce known race layouts, later 3-D with jumps and placeable models. In-game editor as a separate mode, not a second project.
