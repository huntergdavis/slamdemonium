...
## Next slice, as set by the CTO (2026-09-24)

Ordered. Starts after the work already in flight: the true-curve rebuild (Codex) and the half-pipe wall height.

- [ ] NS1 | unassigned | **Accelerator triangles, boost economy, and two persistent instruments (CTO).** Triangles painted on the ground that give a speed kick and charge boost when driven over. Boost charges MUCH more slowly than today, so the pads are the main way to earn it and routing through one becomes a real line choice. On-screen: a boost bar and a speedometer in **mph** (CTO's preference). Deliberately minimal — "I don't want it too busy". NOTE: these are PERSISTENT instruments like the mini-map, not part of the H-toggle HUD stack; the game boots with the HUD off and he must still see boost and speed.
- [ ] NS2 | unassigned | **A much longer track: three minutes at full speed (CTO).** Roughly 10 km at cruise. TENSION TO RESOLVE FIRST: the measured streaming ceiling is 256 props at 1.90 ms p99 against a 2 ms gate. 256 props over 10 km is one every 40 m — an empty road. Either raise the ceiling (needs the contact-island cost addressed, not tuning) or design for dense pockets separated by open running. Bring the CTO the choice with numbers; do not just build a long empty road.
- [ ] NS3 | unassigned | **First game type: a timed run (CTO).** Chosen over racing because "we don't have many of the elements needed for racing in particular yet" — no opponents, no grid, no laps. Countdown, timed run, score, instant retry. This is the run loop that has been parked all day, and it is what turns the toybox into a game with a verb.
- [ ] NS4 | unassigned | **Traffic: other cars to slam into (CTO, implied by the boost economy).** Burnout is the model (CTO confirmed 2026-09-25) and the FAST boost earners are all traffic-dependent: slamming other cars, driving on the wrong side of the road, causing accidents. None can be built today because there is no traffic, so the boost economy ships deliberately incomplete with only slow sources (drift, airtime, pads). That makes traffic LOAD-BEARING for boost rather than set dressing. Sizing note: traffic cars are awake dynamic bodies near the player, and the measured limit is contact islands, not object count — 64 touching bodies cost 16.8 ms against a 2 ms gate, and the active promotion cap is 128. Budget traffic against that before designing density.
