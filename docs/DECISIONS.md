...
## Prop pop-in was a colour swap at 90 m, not draw distance; and the 250 m optical limit (2026-09-24)

The CTO asked to "increase the draw distance for small objects to lower the pop-in effect" and suggested view-aware (foveated) streaming. Measured first: nothing is distance-culled. The far layer is one instanced mesh of every record at its authored pose, never frustum- or distance-culled, the camera far plane is 2000 m and fog on the proving ground is still 97 percent at 220 m. Boot streams props into physics at 90 m and out at 130 m (the module's 220/300 defaults are overridden in `main.ts`), with the active cap at 128; simulated with the real streamer along both runways at 60 m/s, every promotion happens at exactly 90 m, none deferred, peak 72 active. What popped was the swap itself: the far instances used the barrier material (grey 0x6d7779) and the promoted physics visual recolours the same 1 m box orange (0xc86432), so at 90 m a 5.7-pixel grey speck turned orange on every prop on every approach. The CTO's decision: match the colours and spend nothing; the far material now takes `BREAKABLE_PROP_COLOR`, and no radius or streaming change was made.

Recorded so nobody re-measures this: a 1 m prop at 1280 x 720 with a 70 degree camera is 15 px at 35 m, 10 px at 50 m, 5 px at 100 m, 3.4 px at 150 m, 2.3 px at 220 m, 1.7 px at 300 m and 1.3 px at 400 m. Beyond about 250 m a 1 m object is under 2 pixels whatever we stream; that is the draw-distance limit for small props, and it is optics, not culling. Along the main runway up to 160 records sit inside a 220 m radius at once against the 128 cap, and 256 inside 300 m, so raising the physics radius to those values would defer promotions (48 and 116 respectively in simulation) without making anything more visible. View-aware streaming was not built; if it is wanted later, the simulation and this note are the starting point. A separate, universal pop remains: shadows exist only inside the 70 m sun shadow box around the car, so every object gains its shadow at 35 m; that is measured next.
...
## The game boots with the HUD off, and the reminder waits at the bottom (2026-09-24)

The CTO wants a clean screen while driving and the reminder waiting whenever he stops. The HUD now boots in `off` mode; the mini-map is HUD-persistent and keeps updating (from #138, unchanged). The driving-screen key strip at the top (Escape: menu, O: Options, H: HUD, and the controller status) is removed outright, not hidden; its keys move to one short line at the bottom centre, "Esc menu · O options · H HUD" (worded for the pad when he last touched the pad: "Start menu · View options · hold LB for commands"), which shows for five seconds on boot, hides, comes back after five seconds of inactivity and stays until he touches anything, then vanishes at once; it never shows while the HUD is on. Timing is a pure state machine in `src/ui/hudHint.ts` (unit-tested without a DOM); boot feeds it the sampled driving input and the action counts every physics step. Three judgement calls, made and reported: it fades (300 ms in, cut to 150 ms out via the same transition) rather than cuts, because a hard cut in the corner of the eye reads as a glitch; a stick or pedal resting under 5 percent counts as inactivity, so a slightly drifting controller cannot keep the reminder away forever, while any keypress counts as input; and the off default applies on every boot, because the HUD mode has never been persisted, so there is no saved preference to override. A fourth, after the CTO caught the strip still on screen: what survives the shortening is the three keys a first-time player needs; the controller connected/disconnected status does not belong in a reminder and stays with the haptics caption in the Options input group, where the pause menu's controller section already covers it. The lesson written beside it: replace the old thing, do not add the new thing next to it.

## Small props: glow makes them visible, the impostor makes them big; the scaler drop is fill and the curved structures, not the props (2026-09-24)

Measured after the far-layer fix (#143), on the built game at 1280 x 720, render scale 0.60 (the floor of the adaptive scaler and the low end of what the CTO's machine reads while driving), HUD off, driving the main runway with the known prop distances beside each frame, four configurations in one run:

| at 112 m, 12-box cluster | baseline                                                | glow 0.35                | impostor 2.5                    | both                         |
| ------------------------ | ------------------------------------------------------- | ------------------------ | ------------------------------- | ---------------------------- |
| what the screen shows    | 2-pixel dark brown strip on grey asphalt, contrast 0.01 | same size, bright orange | 4 to 6 pixels, still dark brown | 6 to 9 pixels, bright orange |
| orange-classified pixels | 12                                                      | 367                      | 114                             | 1087                         |

At 70 m (inside the physics radius, so the impostor is off by design) glow alone takes the cluster from a 3-pixel dark strip (contrast 0.12) to a bright 11-pixel one (826 orange pixels against 61); at 148 m the same ordering holds by eye. Which lever carries the gain: **glow** is what makes a far prop perceptible at all, because the face the camera sees is in the sun's shade and renders dark brown on dark grey; the **impostor** only adds size, which matters for the 4-pixel target at 150 m on a downscaled frame (a 1 m box at 150 m at scale 0.60 is 2.1 pixels; at 2x it is 4.2). Both ship as independent sliders (`propGlow`, `propFarScale`), glow applied identically to the near and far materials so the 90 m swap stays invisible. Defaults: glow 0.35, far scale 2.0. The honest statement of the impostor's lie: a prop is its true size inside 90 m, 1.5 times life at 150 m, twice life from 200 m out, growing from the ground up so its base does not float; set the slider to 1 for truth everywhere. Neither lever adds render cost: glow is a material uniform, the impostor keeps the instance count.

Why the CTO's render scale drops to 0.60 while he drives (0.70 at rest): the adaptive scaler steps down 0.1 every two seconds the smoothed frame time stays over 18 ms and back up under 17 ms, floor 0.60, so 0.60 is the lowest it can go. What costs the time is not the props: they are three instanced draws (far, near, debris) however many there are. The proving ground's curved structures are 494 individual meshes (loops 248, half-pipe 240, ramps 6), each its own draw when in view and drawn again in the 2048 x 2048 shadow pass when inside the shadow box, and the frame renders at device pixel ratio up to 2 with MSAA, so a HiDPI display pays four times the pixels of a 1080p frame at scale 1. Making props more visible therefore cannot push the scale down further; there is no shared cause. Raising the scaler floor is a pixel cost: 0.85 is 2.0 times the pixels of 0.60, so where his frames sit at the 18 ms edge at 0.60 they would sit near 30 ms at 0.85 on the same stretches, a drop from about 55 to about 33 frames per second there. That is an estimate from the scaling law, not a measurement of his GPU; the honest way to buy pixels is fewer draws (instancing the loop and pipe slabs) or a smaller shadow pass, which is the next measurement.

## The sun shadow box grows from 35 m to 80 m (2026-09-24)

Every object gained its shadow at 35 m because the sun's shadow camera was a 70 m square around the car; the CTO confirmed it bothered him. Four candidates were run on the official sustained gate, which all pass (physics p99 1.6 to 1.9 ms) because the gate measures physics CPU and shadows are render cost; the software renderer showed no difference between them either, so the honest cost figure is the shadow pass's draw count, which is GPU-independent: props are one instanced draw in any box, the curved structures are one draw per slab. Counted along both runways: the 35 m box adds on average 3 to 15 structure meshes to the shadow pass (peak 80); 80 m adds 57 to 114 (peak 250, passing a loop or the half-pipe); 150 m adds 107 to 204 (peak 490, the half-pipe and both loops at once). Texels: 3.4 cm at 35 m and 2048, 7.8 at 80, 14.6 at 150, or 7.3 at 150 with a 4096 map that costs four times the shadow fill. The CTO chose 80 m at 2048: beyond 80 m a 1 m prop is under six pixels at the scaler's floor and its shadow could not be seen anyway, 7.8 cm texels still shade a 1 m box cleanly, and the pass grows to a few dozen draws on average, the same order as one loop in view. The wait-for-the-curves option was put to him (a bigger box becomes nearly free once the 494 slab meshes are instanced, Codex's curve work) and he took 80 m now; the size is chosen on today's geometry and is worth re-examining once the slabs are instanced. `TrackOptions.shadow` keeps the box and map size overridable for measurement.

## The chase camera closes in with speed instead of falling back (2026-09-24)

The CTO: "the faster you go, the closer the camera gets, not the opposite. Our dramatic camera needs to zoom in a bit as we speed up. Right now I guess it's the opposite." Measured on the real vehicle, chase preset, settled at each speed, his guess was right. The follow target sits a constant 7.5 m behind the car, but the rig reaches it through a critically damped spring (camFollowTime 0.25 s), and a damped follower behind a target moving at v settles v x 0.25 s behind it: 15 m at top speed, 21 under boost. On top, the lens widens from 70 to 85 degrees at top speed and 95 with boost. Both levers pushed the same way; the lag did most of the damage.

| speed              | before: distance, fov, car height on screen | after (defaults): distance, fov, car height |
| ------------------ | ------------------------------------------- | ------------------------------------------- |
| 0                  | 7.9 m, 70 deg, 13.4 %                       | 7.9 m, 70 deg, 13.4 %                       |
| 20 m/s             | 12.6 m, 73 deg, 8.1 %                       | 7.3 m, 73 deg, 13.9 %                       |
| 40 m/s             | 17.4 m, 78 deg, 5.5 %                       | 6.7 m, 78 deg, 14.2 %                       |
| 60 m/s (top speed) | 22.2 m, 85 deg, 4.0 %                       | 6.1 m, 85 deg, 14.3 %                       |
| 85 m/s (boost)     | 28.1 m, 95 deg, 2.8 %                       | 5.9 m, 95 deg, 13.2 %                       |

Three independent sliders, so he tunes it in motion rather than us reasoning about it: `camSpeedPull` (default 1) cancels that fraction of the speed lag by leading the follow target along the velocity by v x followTime, so at 1 the camera holds its rest distance at any speed while transients keep the same spring; `camSpeedClose` (default 1.5 m) pulls it closer than rest by up to that much at top speed; and `fovSpeedGain` now runs to -30 so the lens can narrow at speed instead of widening. At the defaults the car keeps the same size on screen from rest to boost while the wider lens still adds peripheral motion. The lead applies only while the car moves along the camera's facing direction, so reversing, the hold and the swing (which the CTO called perfect) run on exactly the old path, and the loop up-vector logic is untouched; the reverse and loop camera tests pass unchanged, and the forward-driving camera golden was regenerated deliberately for this reviewed change.

## Accelerator triangles, a slower boost economy, and two persistent instruments (2026-09-25)

The CTO's next slice (BACKLOG NS1). Triangles painted on the ground give a speed kick and charge boost when driven over; boost charges much more slowly from drifting than it did, so the pads are the main way to earn it and routing through one is a real line choice; a boost bar and a speedometer in mph sit on screen as persistent instruments, like the mini-map, outside the H-toggle stack, because the game boots with the HUD off and he must still see both. Nothing else joins the driving screen: he said "I don't want it too busy".

Two feel decisions, made as sliders so he tunes them and we take his numbers: the kick is an **impulse**, `padKick` metres per second added along the heading the moment the car enters the triangle (default 8), not a temporary top-speed lift, because an impulse is felt at every speed, is over in an instant, and is worth a small detour without removing the choice, whereas a top-speed lift does nothing below top speed and would make pads pointless for most of a lap; and one pad grants `padBoost` of the bar (default 0.35, three pads from empty to full). `driftChargeRate` drops from 0.35 to 0.1 (three and a half times slower), the change that gives the pads their meaning; the replay fixtures' headers carry the new 0.1 because verify-mode replays require the header to match the defaults, and none of the three scripts ever presses boost, so the meter never touches their motion and their recorded results do not move. A pad fires once per visit, on entry, and re-arms when the car leaves, so sitting on one or crawling across it cannot farm it; a respawn re-arms every pad. Pads are `src/world/boostPads.ts`: paint (one self-lit mesh) and a point-in-triangle footprint test, no physics body. The proving ground has four, 12 m long and 6 m wide, set 10 m off the main runway's centreline on alternating sides and one on the east branch, so hitting one is a deliberate lane change. The instruments are one card beside the mini-map: speed in mph and the boost bar, updated in every HUD mode including off and collapsed; the full HUD's own speed card stays in km/h for the tuning lab.

## Three things #147 showed on screen that the code said it did not (2026-09-25)

The CTO drove the #147 preview and could not find the accelerator triangles. The PM's own spawn capture showed two more things missing: the mini-map and the new speed and boost card. All three were found by looking at frames and the DOM of the hosted build, not by reading the mount code, after the day's earlier lesson (far props at the origin). Each is a defect with a frame-level test now.

**The persistent instruments were below the bottom of the screen on any viewport 820 px tall or shorter.** The stylesheet has a short-viewport layout that turns the HUD into a bottom-anchored dock, sized by its contents. With the HUD off at boot (#140), the dock's only in-flow content is the reminder line, so it is 72 px tall, and the mini-map and drive card, positioned absolutely inside it at 72 px and 300 px down, hang off its bottom edge. Measured on the hosted build at 1280 by 720: dock at y 632, mini-map at y 704, card at y 932. At 1280 by 900 the layout is the desktop one and both are on screen, which is why it passed when the HUD booted full (#134, #138) and why Playwright's visibility check passed: it tests rendered, not on screen. Fix: in the short-viewport layout the two instruments are pinned to the viewport's top-right, independent of the dock. The boot e2e now asserts both are fully in the viewport at 720 px. Introduced by #140; the mini-map has been missing on short screens since it merged.

**The pads were drawn face down.** The triangle was wound left, apex, right, which seen from above is clockwise; three.js treats counter-clockwise as the front face and the material culls back faces, so every pad was in the scene graph at the right place and in no frame. The read-back tests checked positions, colour and height, never the normal. Now wound left, right, apex; the unit test asserts the face normal points up with FrontSide culling on.

**One pad now sits on the spawn runway centreline, 40 m ahead of spawn.** Even drawn correctly, the nearest pad was 60 m ahead and 10 m to the left, a thin sliver from the chase camera. The PM asked for one the CTO cannot miss so he can judge the mechanic; the other four stay off the line as line choices. It moves once the kick and economy are settled.

## Boost is earned by danger: the economy, the sources that do not exist yet, and the B key (2026-09-25)

The CTO, driving #147 with drift charge already 3.5 times slower than before: "the boost is still filling too fast, it needs to be a slow fill", and then the design itself: "pads shouldn't charge much if any, just a tiny percentage. Same with drifting, drifting should charge but very slowly. It's slamming other cars, driving on wrong side of road, causing accidents, that will eventually make it fill faster. For testing, we should just have a button we can press to auto-fill it. In-game it should be a slow fill for everything we have now (airtime should also slowly fill it like flying off a ramp)."

**What adds to the bar today, measured.** Exactly two sources existed: drift charge and the pad grant. There is no passive trickle, no refill on respawn (respawn zeroes the bar), no grant from landings, crashes, scoring or chains. `setDriftMeter` is the test API only. Over the three recorded example scripts with real physics: standing-start and ring-lap earn 0.000 (no drift over 12 degrees, no flight); handbrake-turn earns 0.030 from 0.59 s of charging at the old 0.1 rate. Those scripts are not how the CTO drives; at the reference drift (30 degrees at 40 m/s) the old rate filled the bar in 10 s, a realistic 20 degree slide at 30 m/s in 20 s, so a minute with half of it sideways was about 1.5 bars. Plus three pads to a bar. That is "fills fast".
