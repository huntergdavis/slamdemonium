...
describe('tuning schema: design sections 7.2 and 13.1', () => {
  it('has all 71 unique keys, valid ranges, labels, units, and help', () => {
    expect(PARAM_DEFS).toHaveLength(71);
  ...
  it('matches every documented value, unit, flag and help string exactly', () => {
    const brief = readFileSync(
      new URL('../docs/vertical-slice-design.md', import.meta.url),
      'utf8',
    );
    const table =
    ...
    for (const row of rows) {
      const cells = row
        ...
        .slice(1, -1)
        .map((cell) => cell.trim());
      const [key = '', group, unit, initial, min, max, step, flags = '', help] =
        cells;
  ...
  it('pins exactly the 14 specified Quick Tune controls', () => {
    expect(
      PARAM_DEFS.filter((definition) => definition.quick).map(
  ...
  it('ships Default, Grip, Drifty and Raw with known, in-range overrides', () => {
    expect(Object.keys(BUILTIN_PRESETS)).toEqual([
      'Default',
      'Grip',
      'Drifty',
      'Raw',
    ...
    expect(Object.keys(DEFAULT_VALUES)).toHaveLength(71);
